...
final class Analytics_Tests: XCTestCase {
    ...
    func testTrackPerCallContextSchemaVersion() {
        let analytics = Analytics(configuration: Configuration(writeKey: "testTrackPerCallContextSchemaVersion"))
        let outputReader = OutputReaderPlugin()
        analytics.add(plugin: outputReader)
    ...
    func testOverlappingAliasContextDoesNotSwap() {
        let analytics = Analytics(configuration: Configuration(writeKey: "testOverlappingAliasContextDoesNotSwap"))
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            analytics.alias(newId: "user-a", context: [
                "protocols": ["schemaVersion": "v1"]
            ])
            group.leave()
        }
        DispatchQueue.global(qos: .userInitiated).async {
            analytics.alias(newId: "user-b", context: [
        ...
        let aliases = collector.events.compactMap { $0 as? AliasEvent }
        XCTAssertEqual(aliases.count, 2)

        let aliasA = aliases.first { $0.userId == "user-a" }
        let aliasB = aliases.first { $0.userId == "user-b" }
        XCTAssertNotNil(aliasA)
        XCTAssertNotNil(aliasB)
        XCTAssertEqual(aliasA?.context?.dictionaryValue?[keyPath: "protocols.schemaVersion"] as? String, "v1")
        XCTAssertEqual(aliasB?.context?.dictionaryValue?[keyPath: "protocols.schemaVersion"] as? String, "v2")
    }

    func testExistingEventCallSitesStillCompile() {
        let analytics = Analytics(configuration: Configuration(writeKey: "testExistingEventCallSitesStillCompile"))
        let collector = AllEventsReaderPlugin()
        analytics.add(plugin: collector)

        waitUntilStarted(analytics: analytics)

        analytics.track(name: "plain")
        analytics.track(name: "with props", properties: ["k": "v"])
        analytics.track(name: "typed props", properties: MyTraits(email: "a@b.c"))
        analytics.identify(userId: "u1")
        analytics.identify(userId: "u2", traits: MyTraits(email: "a@b.c"))
        analytics.identify(traits: MyTraits(email: "a@b.c"))
        analytics.screen(title: "home")
        analytics.screen(title: "home", category: "app")
        analytics.screen(title: "home", category: "app", properties: MyTraits(email: "a@b.c"))
        analytics.group(groupId: "g1")
        analytics.group(groupId: "g1", traits: MyTraits(email: "a@b.c"))
        analytics.alias(newId: "u3")

        XCTAssertEqual(collector.events.count, 12)
    }

    func testPerCallContextOnAllEventTypes() {
        let analytics = Analytics(configuration: Configuration(writeKey: "testPerCallContextOnAllEventTypes"))
        let collector = AllEventsReaderPlugin()
        analytics.add(plugin: collector)

        waitUntilStarted(analytics: analytics)

        analytics.screen(title: "home", context: ["protocols": ["schemaVersion": "v1"]])
        analytics.identify(userId: "u", traits: ["email": "a@b.c"], context: ["protocols": ["schemaVersion": "v1"]])
        analytics.group(groupId: "g", traits: ["name": "org"], context: ["protocols": ["schemaVersion": "v1"]])
        analytics.alias(newId: "u2", context: ["protocols": ["schemaVersion": "v1"]])
